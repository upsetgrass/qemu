...
static void plugin_exit(qemu_plugin_id_t id, void *p)
{
    ...
    // unsigned int - %u   size_t - %zu   uint64_t - %" PRIu64 " 
    g_string_printf(msg, "vcpu_num=%d\nstatic_counter:%u\ndynamoic_counter:%"PRIu64"\n", vcpu_num, g_hash_table_size(insn_addrs), val);
...
QEMU_PLUGIN_EXPORT int qemu_plugin_install(qemu_plugin_id_t id, const qemu_info_t *info, int argc, char **argv)
{
    ...
    insn_count = qemu_plugin_scoreboard_u64(qemu_plugin_scoreboard_new(sizeof(uint64_t))); // 提供一个{score,0} qemu_plugin_scoreboard*
    // start register
...
/*
    为什么user-mode是单核，用普通变量dynamic++得到的结果比qemu_plugin_u64_add(insn_count, vcpu_index, 1)的结果小的多
*/